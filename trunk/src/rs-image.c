...
RS_IMAGE16 *
rs_image16_scale_affine(RS_IMAGE16 *in, RS_IMAGE16 *out, gdouble scale)
{
	RS_MATRIX3 mat;

	matrix3_identity(&mat);
	matrix3_affine_scale(&mat, scale, scale);
//	matrix3_affine_rotate(&mat, 10.0);
	out = rs_image16_affine(in, out, &mat);
	return(out);
}
